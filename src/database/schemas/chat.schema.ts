...
export const chatMessages = pgTable(
  ...
  (table) => ({
    ...
    courseTimeIdx: index('chat_messages_course_time_idx').on(
      table.courseId,
      table.createdAt,
    ),
    ...
    parentMessageIdx: index('chat_messages_parent_idx').on(
      table.parentMessageId,
    ),
    clientMessageIdx: index('chat_messages_client_idx').on(
...
export const chatMessageReads = pgTable(
  ...
  (table) => ({
    ...
    messageUserIdx: index('chat_message_reads_message_user_idx').on(
      table.messageId,
      table.userId,
    ),
    userCourseIdx: index('chat_message_reads_user_course_idx').on(
      table.userId,
      table.courseId,
    ),
...
export const chatRoomMembers = pgTable(
  'chat_room_members',
  {
    ...
    // Permissions
    canSendMessages: varchar('can_send_messages', { length: 10 })
      .notNull()
      .default('true'),
    canSendImages: varchar('can_send_images', { length: 10 })
      .notNull()
      .default('true'),
    canDeleteOwnMessages: varchar('can_delete_own_messages', { length: 10 })
      .notNull()
      .default('true'),
  ...
  (table) => ({
    ...
    courseUserIdx: index('chat_room_members_course_user_idx').on(
      table.courseId,
      table.userId,
    ),
