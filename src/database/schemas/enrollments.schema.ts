...
export const enrollments = pgTable(
  'enrollments',
  {
    ...
    // Enrollment metadata
    enrollmentSource: varchar('enrollment_source', { length: 50 }).default(
      'web',
    ), // web, mobile, admin
  ...
  (table) => ({
    ...
    studentCourseIdx: index('enrollments_student_course_idx').on(
      table.studentId,
      table.courseId,
    ),
    ...
    accessExpiryIdx: index('enrollments_access_expiry_idx').on(
