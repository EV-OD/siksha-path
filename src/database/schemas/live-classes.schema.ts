...
export const liveClasses = pgTable(
  'live_classes',
  {
    ...
    timezone: varchar('timezone', { length: 50 }).notNull().default('Asia/Kathmandu'),
    ...
    recordingEnabled: varchar('recording_enabled', { length: 10 }).notNull().default('true'),
  ...
  (table) => ({
    ...
    scheduledTimeIdx: index('live_classes_scheduled_time_idx').on(table.scheduledStartTime),
    zoomMeetingIdx: index('live_classes_zoom_meeting_idx').on(table.zoomMeetingId),
  })
);
...
export const liveClassAttendance = pgTable(
  'live_class_attendance',
  {
    ...
    attendanceStatus: varchar('attendance_status', { length: 20 }).notNull().default('absent'), // present, absent, late
  ...
  (table) => ({
    ...
    liveClassStudentIdx: index('live_class_attendance_class_student_idx').on(table.liveClassId, table.studentId),
    liveClassIdx: index('live_class_attendance_class_idx').on(table.liveClassId),
 ...
 */
export const liveClassAttendanceRelations = relations(liveClassAttendance, ({ one }) => ({
